...
export default function InstagramVideoDownloadHelper() {
  ...
  const extractVideo = useAction(api.toolsActions.extractInstagramVideo);
  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-14">
        <ToolsBreadcrumb toolName="Instagram Video Download Helper" toolHref="/tools/instagram-video-downloader" />
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-pink-50 text-pink-600 border border-pink-100 px-3 py-1 rounded-full text-xs font-semibold mb-4">
             <Image src="/icons/instagram.png" alt="Instagram" width={14} height={14} className="object-contain" /> Free Tool
          </div>
          <h1 className="text-4xl font-extrabold tracking-tight mb-3 italic">
            Instagram Video Download Helper
          </h1>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            Paste a public Instagram Reel, post, or video link and we&apos;ll try to fetch a downloadable file. This free helper depends on public extraction services, so some links may fail.
          </p>
        </div>
          ...
          <form onSubmit={handleExtract} className="relative flex flex-col gap-6">
            <div className="space-y-4">
              <label className="flex items-center gap-2 text-sm font-bold text-muted-foreground uppercase tracking-wider">
                <LinkIcon size={16} className="text-primary" /> Paste Instagram Link
              </label>
              ...
              <div className="mt-4 p-6 bg-secondary/20 rounded-3xl border border-dashed border-border animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex flex-col items-center text-center gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-muted-foreground">A public extraction service returned a downloadable file for this Instagram link.</p>
                  </div>

                  {result.url && (
                    <button
                      onClick={() => void handleDownload(result.url!, "instagram-video.mp4", "main")}
                  ...
                  {result.images && (
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 w-full">
                      {result.images.map((img, idx) => (
                        <button
                          key={idx}
                          onClick={() => void handleDownload(img, `instagram-image-${idx + 1}.jpg`, idx)}
        ...
        <div className="grid md:grid-cols-3 gap-8 mb-16">
          ...
          <div className="bg-white border border-border rounded-3xl p-8 shadow-sm">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              Private, deleted, or unsupported Reels and posts may not work with any free downloader.
            </p>
          </div>
          <div className="bg-white border border-border rounded-3xl p-8 shadow-sm">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">
              No account required, but free extraction can be unstable. For saving your own videos, Instagram&apos;s built-in save/share options are the safest fallback.
            </p>
          </div>
        </div>
