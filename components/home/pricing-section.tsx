...
function PricingSection() {
  ...
  const plans = [
    {
      name: "Starter",
      desc: "Perfect for testing the waters and seeing the AI magic.",
      monthlyPrice: "$0",
      yearlyPrice: "$0",
      features: ["3 Projects per month", "Export in 720p", "Basic AI clipping"],
      cta: "Get Started Free",
      highlighted: false,
    },
    {
      name: "Pro Creator",
      desc: "For serious creators who want to dominate the algorithm.",
      monthlyPrice: "$29",
      yearlyPrice: "$24",
      features: ["30 Projects per month", "Export in 4K resolution", "Advanced viral hooks", "Auto-captions"],
      cta: "Start 7-Day Trial",
      highlighted: true,
    },
    {
      name: "Agency",
      desc: "Scale your content production effortlessly for multiple clients.",
      monthlyPrice: "$99",
      yearlyPrice: "$82",
      features: ["Unlimited projects", "Custom brand templates", "Team collaboration", "Priority support"],
      cta: "Contact Sales",
      highlighted: false,
    }
  ];
    ...
    <section id="pricing" className="py-24 bg-background">
      ...
      >
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="text-lg text-muted-foreground mb-8">Unlock the full potential of your content. No hidden fees, cancel anytime.</p>
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <span className={cn("text-lg font-bold transition-colors flex items-center gap-2", isYearly ? "text-foreground" : "text-muted-foreground")}>
              Yearly <span className="text-xs bg-[#A8E6A1] text-green-950 px-2 py-0.5 rounded-full font-extrabold border border-green-300 shadow-sm">Save 17%</span>
            </span>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8 items-center">
          {plans.map((plan, i) => (
             <div key={i} className={`rounded-[2rem] p-8 w-full transition-all duration-300 relative ${plan.highlighted ? 'bg-primary text-primary-foreground shadow-2xl scale-100 md:scale-105 border-transparent z-10' : 'bg-white text-foreground border border-border shadow-sm hover:shadow-xl hover:-translate-y-1'}`}>
              {plan.highlighted && (
              ...
              <p className={`text-sm mb-6 h-10 ${plan.highlighted ? 'text-primary-foreground/80' : 'text-muted-foreground'}`}>{plan.desc}</p>
              <div className="mb-6">
                <div className="flex items-end gap-1">
                  <span className="text-5xl font-extrabold">{isYearly ? plan.yearlyPrice : plan.monthlyPrice}</span>
                  <span className={`text-sm font-medium ${plan.highlighted ? 'text-primary-foreground/80' : 'text-muted-foreground'} mb-1`}>/mo</span>
                </div>
                {isYearly && plan.name !== "Starter" ? (
                  ...
                  <div className={`mt-2 text-[11px] font-semibold uppercase tracking-wider opacity-0`}>
                    Placeholder
                  </div>
                )}
              </div>
              ...
              <ul className="space-y-4 mb-8">
                {plan.features.map((feature, j) => (
                  <li key={j} className="flex gap-3 items-center">
                    <CheckCircle2 size={18} className={`shrink-0 ${plan.highlighted ? 'text-accent' : 'text-primary'}`} />
