        ...
        <div className="flex text-yellow-500 mb-6 drop-shadow-sm transition-transform group-hover:scale-105 origin-left">
          {[1, 2, 3, 4, 5].map((s) => (
            <Star
              ...
              className={s <= rating ? "" : "text-border"}
            />
          ))}
        </div>
        <p className="text-lg text-foreground mb-8 font-medium leading-relaxed">
          &quot;{quote}&quot;
        </p>
      </div>
...
function SocialProofSection() {
  ...
  return (
    <section
      id="testimonials"
      className="py-24 bg-white border-y border-border/40"
    >
      <motion.div
      ...
      >
        ...
        <p className="text-base sm:text-xl text-muted-foreground mb-8 md:mb-16 max-w-2xl mx-auto leading-relaxed">
          From solo podcasters to content agencies — ShortPurify helps you turn
          long videos into scroll-stopping shorts without the grind.
        </p>

        {/* Platform Trust Marquee */}
        <div className="mb-10 md:mb-20">
          <p className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground/60 mb-8">
